...
def run_simulation(
    ...
    Args:
        ...
        takeoff_flap_setting: Takeoff flap setting (0 for 0Â°, 1 for 15Â°).
    ...
    # Tri-level bias mid point and function (use provided mid if given)
    bias_alt_mid = float(bias_alt_mid) if bias_alt_mid is not None else (bias_alt_low + bias_alt_high) / 2.0
    sfc_bias_mid_frac = (float(sfc_bias_mid) / 100.0) if (sfc_bias_mid is not None) else (sfc_bias_low_frac + sfc_bias_high_frac) / 2.0
    thrust_bias_mid_frac = (float(thrust_bias_mid) / 100.0) if (thrust_bias_mid is not None) else (thrust_bias_low_frac + thrust_bias_high_frac) / 2.0

    def _interp_bias3(alt_ft: float, low_frac: float, mid_frac: float, high_frac: float,
                      alt_low: float, alt_mid: float, alt_high: float) -> float:
        if alt_ft <= alt_low:
            return low_frac
        if alt_ft >= alt_high:
            return high_frac
        if alt_ft <= alt_mid:
            span = max(1.0, alt_mid - alt_low)
            frac = (alt_ft - alt_low) / span
            return low_frac + frac * (mid_frac - low_frac)
        span = max(1.0, alt_high - alt_mid)
        frac = (alt_ft - alt_mid) / span
        return mid_frac + frac * (high_frac - mid_frac)
